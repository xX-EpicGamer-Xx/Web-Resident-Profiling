...
if ( $_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    do {
        ...
        $sql = "INSERT INTO residents (
                ...
                pwd, pwd_id_no, single_parent, house_number, subdivision, street,
...
<head>
    ...
    <style>
        ...
        #last-name,#first-name, #member-4ps, #middle-name,#suffix, #birth-date,#birth-place,#age,#civil-status,#nationality,#religion,#occupation,#contact-number-general,#pwd,#pwd-id-no,#single-parent,#house-number,#subdivision,#street,#registered-voter,#voter-s-id-number,#tin-number,#national-id-number,#sss-number,#philhealth-no,#pagibig-no,#vaccinated {
            position: absolute;
            width: 13%;
            height: 5%;
            background: transparent; 
...
<body>
    <div class="container my-5" id="main-tab">
        ...
        <form method="post">
            ...
            <a class="btn btn-outline-primary" href="/account/index.php" role="button" id="cancel-button"> Cancel </a>  
        </form>
        </div>
    </div>
</body>
